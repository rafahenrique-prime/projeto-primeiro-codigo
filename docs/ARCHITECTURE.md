...
│   ├── qwen-health.js          ← health check do QwenCloud p/ Operations Center (Fase 2A) — só disponibilidade/latência, nunca saldo/quota
...
### Abas do módulo Cobranças — separação Clientes × Parcelas (CobrancasPage.jsx, Fase 2 — 2026-07-15)

Estrutura atual: `👤 Clientes em Cobrança | 📊 Dashboard | 📈 Fluxo de Caixa | 📅 Histórico | 👥 Clientes | 💳 Parcelas`.

A antiga aba "📋 Lista" (1 linha por Parcela, redundante com a aba Parcelas da Fase 1) foi **renomeada e reestruturada** para "👤 Clientes em Cobrança" — agora responde "qual a situação financeira geral de cada cliente?" (1 card por cliente), enquanto "💳 Parcelas" continua respondendo "qual a situação de cada parcela?" (1 linha por parcela, sem mudança estrutural nesta fase). "👥 Clientes" continua sendo só cadastro/relacionamento.

- **Componente novo:** `src/components/cobrancas/ClientesEmCobrancaTab.jsx` — segundo componente do módulo extraído para fora de `CobrancasPage.jsx` (o primeiro foi `ParcelasTab.jsx` na Fase 1). A antiga UI/lógica de filtro da aba Lista foi removida de `CobrancasPage.jsx` (estados `filter`/`search`/`sortBy`, `filterDefs`, `FilterChip`, e os helpers `statusColor`/`WhatsAppIcon` que só eram usados ali).
- **Fonte de dados:** reaproveita 100% o mesmo state `cobrancas` (vindo de `getAllCobrancas()`) já usado por `ParcelasTab.jsx` — **nenhuma chamada de API nova**, **nenhuma escrita no Base44**, `cobrancasService.js` não foi alterado.
- **Agregação por cliente:** feita 100% na camada de apresentação (`agruparPorCliente()` dentro do componente). Chave de agrupamento: `clienteId` sempre que existir; fallback só quando ausente: `nome normalizado (sem acento) + telefone normalizado (só dígitos)` — evita juntar todos os "sem nome" num único cliente e reduz risco de homônimos. Um mesmo nome com `clienteId` diferentes gera cards separados (chave prioriza sempre o ID).
- **Status consolidado** (prioridade fixa, sem sobreposição — ver `ClientesEmCobrancaTab.jsx`):
  1. **Quitado** — `valorAbertoTotal <= 0` (com epsilon de 0.005 pra arredondamento) ou todas as parcelas pagas.
  2. **Vencido** — existe ≥1 parcela com `diasAtraso > 0` e `valorAberto > 0` (e não é quitado).
  3. **Parcial** — `valorPagoTotal > 0` e `valorAbertoTotal > 0`, e não é vencido nem quitado.
  4. **Pendente** — os demais casos (`valorAbertoTotal > 0`, nenhuma parcela vencida, `valorPagoTotal === 0`).
  5. **Crítico** — flag visual complementar (não é status principal), ligada quando `maiorAtraso > 15` dias — mesmo limiar já usado em `getTotalizadores()` (`cobrancasService.js`) e nos filtros da aba Parcelas.
- **Progresso financeiro:** `percentual = valorPagoTotal / valorTotalTotal * 100` (por **valor**, não por quantidade de parcelas — mais confiável porque os dois campos nunca são `null`). Cai pra 0% só se `valorTotalTotal` for 0 (proteção contra divisão por zero, não deveria ocorrer). Calculado só na camada de apresentação, nunca gravado no Base44.
- **Campos exibidos por cliente:** nome, telefone, status + badge crítico, `X/Y parcelas pagas` (+ pendentes/vencidas), maior atraso (só considera parcelas ainda em aberto), próximo vencimento (idem), valor total/pago/restante, barra de progresso, botão "💳 Ver parcelas".
- **Navegação "💳 Ver parcelas":** `CobrancasPage.jsx` guarda um state `initialParcelasFilter` (`{ clienteId, nome } | null`). O clique chama `handleVerParcelas()`, que seta esse filtro e troca pra aba Parcelas. `ParcelasTab.jsx` ganhou as props `initialClienteFilter`/`onClearInitialFilter`: filtra por `clienteId` quando presente (nunca só por nome se o ID existir), com fallback por nome só quando o cliente não tinha `clienteId`. Um banner "Mostrando parcelas de: X" com botão "✕ Limpar filtro" aparece nesse caso. Clicar diretamente na aba "💳 Parcelas" (sem passar por "Ver parcelas") sempre limpa esse filtro antes de trocar de aba, pra nunca reaproveitar um filtro antigo por engano.
- **"👤 Ver cliente" (abrir ficha no CRM): não implementado nesta fase.** Investigado na auditoria — a página de Contatos (`ContactsPage`) é indexada por conversa do Inbox, não existe hoje um mapeamento confiável entre `cliente_id` (Base44 PRIME STORE - COBRANÇAS) e o contato/conversa correspondente. Fica pendente de decisão de produto numa fase futura.
- **Ação "Registrar pagamento manual" (💰) órfã:** a antiga aba Lista tinha botões de WhatsApp, copiar link do portal e registrar pagamento manual (que chamava `registrarPagamentoManual()`). Os dois primeiros foram removidos (eram específicos da visão por parcela). O terceiro — `PagamentoModal`, o state `pagamentoModalCobranca` e o import de `registrarPagamentoManual` em `CobrancasPage.jsx` — **foi mantido no código, mas hoje não tem nenhum botão que o acione** (ficou sem trigger na UI). Decisão pendente: re-wire dentro de `ParcelasTab.jsx` (onde faz sentido, 1 parcela = 1 pagamento) numa fase futura aprovada, ou remover de vez.
- **Limitação herdada do Fluxo F:** "parcelamento real" (N parcelas por venda) não é gerado automaticamente pelo sync Lyra→PRIME — mas já é exibido corretamente quando existe mais de 1 Parcela pro mesmo `cliente_id` (confirmado em produção com clientes reais de 2, 3 e 4 parcelas).
...
**CORS (Fase 3.2.0 — 2026-07-14):** `gerar-cobranca-lyra` suporta preflight `OPTIONS` (necessário pra ser chamado direto do navegador, do futuro service no PRIME COBRANÇAS/Base44). Allowlist por **igualdade exata** contra `GERAR_COBRANCA_ALLOWED_ORIGINS` (nunca `startsWith`/`includes` — evita aceitar `https://prime-vip.base44.app.evil.com` ou variações). Headers `Authorization` e `Content-Type` liberados via `Access-Control-Allow-Headers`. O wildcard `Access-Control-Allow-Origin: *` global (aplicado a todas as tools no topo do handler) é removido **só dentro deste `case`**, substituído pela origem exata quando permitida — as outras 4 tools (`vercel-status`, `sync-lyra`, `lyra-webhook`, `stuck-check`) continuam com o `*` de sempre, sem nenhuma mudança de comportamento.

**Autenticação dupla — modo admin e modo frontend (Fase 3.3.1/3.3.2 — 2026-07-14):** o Base44 Starter não expõe Secrets do backend pro frontend React/Vite, então `GERAR_COBRANCA_SECRET` não pode ir pro bundle do PRIME COBRANÇAS. Criado um segundo segredo, `COBRANCA_FRONTEND_TOKEN`, **temporário e conscientemente público** no bundle — só autoriza esta tool (nenhuma outra), só aceita `dryRun=false` com body estritamente `{parcela_id, dryRun}` (rejeita qualquer campo extra), exige Origin exato (não best-effort) e tem rate limit próprio, mais restritivo (5/min por IP, 2/5min por parcela — Maps separados do modo admin, best-effort, resetam a cada cold start, não substituem autenticação real). A lógica financeira (`gerarCobrancaLyraReal`) é 100% compartilhada entre os dois modos, sem nenhuma alteração — sempre relê Parcela/Venda/Cliente oficialmente do PRIME, nunca confia em dado vindo do request. **Risco assumido conscientemente:** qualquer token num bundle público pode ser extraído via DevTools — por isso o app PRIME STORE - COBRANÇAS deve permanecer restrito a usuários convidados no Base44 antes de este token ser usado de fato no frontend, e a solução é só temporária. Revogação: remover `COBRANCA_FRONTEND_TOKEN` da Vercel desativa só o modo frontend, sem afetar `GERAR_COBRANCA_SECRET`/`sync-lyra`/`lyra-webhook`/cobranças já criadas.
...
### Fluxo H — WhatsApp Premium (envio de mensagens de cobrança, 2026-07-29)

Novo ponto de entrada de envio manual dentro do módulo Cobranças, documentado em detalhe em [`docs/WhatsApp-Premium.md`](WhatsApp-Premium.md). Resumo arquitetural:

```
Botão WhatsApp (ClientesEmCobrancaTab.jsx / ParcelasTab.jsx / CobrancasPage.jsx)
        ↓
WhatsAppSendModal.jsx — "Mensagem pronta" (template) ou "Mensagem personalizada"
(a opção personalizada delega 100% pro EnviarMensagemManualModal.jsx já existente, sem alteração)
        ↓
cobrancasService.js — listarTemplatesWhatsapp() / previsualizarMensagemWhatsapp() / enviarMensagemManual()
        ↓
POST /api/system-tools?tool=mensagem-manual (dispatcher — Origin allowlist + rate-limit)
        ↓
api/_mensagemManualProxy.js (helper privado, mesmo padrão de _gerarCobrancaLyra.js) —
valida payload por allowlist estrita, nunca aceita telefone/template_key/dados financeiros do navegador
        ↓
enviarMensagemManualWhatsapp (Base44, PRIME STORE - COBRANÇAS) → whatsappProvider → ZAP-API → WhatsApp do cliente
```

**Reaproveitamento, não duplicação:** o envio de fato (`chamarEnviarMensagemManualWhatsapp`) já existia e não foi alterado — só ganhou dois vizinhos (`chamarListarTemplates`/`chamarPrevisualizarMensagem`) que usam a mesma URL/token/timeout, interceptados antes da validação de envio via campo `acao` no corpo. Idempotência por `request_id` (UUID, dedup em memória da instância) e rate-limit best-effort por IP são os mesmos mecanismos já documentados no Fluxo F.1 — nenhuma trava distribuída real.

**Validado em produção (2026-07-29):** fluxo completo testado contra o backend real (`GET`/`POST` retornando 200 em todas as chamadas — listar templates, prévia, envio) com um cliente de teste explicitamente identificado (`RAFAEL TESTE`), nas duas variantes (template e personalizada), sem erro de console nem duplicidade de requisição. Detalhes completos, limitações conhecidas e histórico de commits em `docs/WhatsApp-Premium.md`.

### Fluxo I — Integração NEX (importação de clientes, Fase 6C — 2026-07-31)

Novo fluxo de sincronização, com direção inversa aos demais: um sistema Windows externo (`PrimeIntegracaoNex`, fora deste repositório) chama o IGNITE PRIME, não o contrário. Detalhe completo (schema, segurança, endpoints, testes) em [`docs/integrations/NEX-INTEGRATION.md`](integrations/NEX-INTEGRATION.md) e `docs/SUPABASE.md §3.7` (referência textual, mesmo padrão já usado no resto deste documento — sem link de anchor, por convenção do projeto).

```
PrimeIntegracaoNex (Windows, fora do repo)
        ↓ POST com Bearer NEX_SYNC_SECRET
api/system-tools.js?tool=nex-sync-clientes
        (também: ?tool=nex-cliente — mesma auth; ?tool=nex-health — público, exceto ?force=true)
        ↓ delega 100% (validação de HTTP só, zero lógica de negócio)
api/_nexClientes.js — processarLote()
        ↓
Supabase: nex_clientes + nex_sync_eventos (staging isolado, RLS zero-policy)
```

**Isolamento do Base44:** nenhuma tabela NEX nem `api/_nexClientes.js` referenciam `Cliente`/`Venda`/`Parcela`/`HistoricoAtividade` ou o SDK Base44 — confirmado por auditoria estrutural (grep) antes do deploy.

**Idempotência:** `content_hash` (SHA256 do payload normalizado, exclui a chave natural `origem_loja`+`nex_codigo`) classifica cada cliente como `criado`/`atualizado`/`sem_alteracao` sem comparar campo a campo — mesma filosofia de atomicidade "por construção" já usada em `claim_qwen_health_check` (migration 015), embora o mecanismo seja diferente (hash determinístico vs. `UPDATE ... RETURNING` atômico).

**Limite de 500 registros por chamada** — `processarLote` rejeita lotes maiores com erro estruturado, sem processamento parcial.

**Segurança:** `NEX_SYNC_SECRET` isolado de todos os outros segredos do projeto, configurado somente em Vercel Production (nunca Preview, nunca `.env.local`, nunca `VITE_`), com cópia de recuperação em cofre local protegido do macOS. RLS zero-policy em ambas as tabelas (ver `docs/SUPABASE.md §4`).

**Migration aplicada em 2026-07-31, código corrigido e publicado em 2026-08-01:** `016_nex_clientes.sql` foi executada manualmente no SQL Editor do Supabase Production (`Success. No rows returned`) — tabelas criadas vazias, RLS habilitada, validação estrutural feita antes de avançar.

**Bug de Production e correção (Fase 6C.6):** o primeiro deploy do código (commit `faffb99`) chegou com um bug — os 3 handlers instanciavam `createClient(SUPABASE_URL, SUPABASE_SECRET_KEY)`, mas `createClient` já estava importado no topo do arquivo a partir do `@base44/sdk` (usado por Lyra/PRIME Cobranças) — o nome compartilhado fez o código NEX chamar por engano o client errado, gerando `500`/`"supabase.from is not a function"` em Production. Corrigido no commit `a595ab5`: acesso 100% via REST/`fetch` (mesmo padrão de `_profileLearning.js`/`qwenHealthSupabaseHeaders()`), toda a lógica concentrada em `api/_nexClientes.js`, zero uso de `createClient` (Base44 ou Supabase-JS) no fluxo NEX. Detalhe completo em `docs/integrations/NEX-INTEGRATION.md §5`.

**Zero Serverless Functions novas** — os 3 endpoints (`nex-sync-clientes`, `nex-cliente`, `nex-health`) são `case` dentro do dispatcher já existente `api/system-tools.js`, mesmo padrão de consolidação de `qwen-health`/`sync-lyra`/`stuck-check` para respeitar o teto de 12 funções do plano Hobby — contagem de `api/` permanece inalterada.

**Testado (2026-08-01):** 70/70 testes PASSING, incluindo cobertura comportamental que exercita os 3 handlers reais (`default export` de `system-tools.js`) e confirma que `createClient` do Base44 nunca é chamado pelo fluxo NEX — impede regressão do bug acima sem depender de deploy pra descobrir. Detalhe em `docs/integrations/NEX-INTEGRATION.md §4`.

**POC validada em Production (Fase 6C.7):** sequência fechada de 7 cenários (criação, atualização, idempotência item-a-item e em lote, erro de validação, consulta individual, agregados) executada com dados exclusivamente fictícios (`origem_loja='primestore-udi-poc-teste'`), encerrada com cleanup confirmado (ambiente devolvido a 0 clientes/0 eventos). Detalhe completo em `docs/integrations/NEX-INTEGRATION.md §6`.

**Pendências (baixa prioridade, não bloqueiam produção):** `updated_at` não é atualizado no upsert; formato exato da `SUPABASE_SECRET_KEY` não confirmado formalmente (mas já validado empiricamente em Production). Ver `docs/integrations/NEX-INTEGRATION.md §8`.

### Fluxo J — Alerta Inteligente de Handoff Humano (homologado, 2026-08-13)

Quando a Gaby decide que uma conversa precisa de atendimento humano, a intention "Alerta rafael" chama `GET /api/system-tools?tool=alerta-inteligente` (`case` em `api/system-tools.js`, lógica em `api/_alertaInteligente.js`). O endpoint localiza o chat real por `whatsappPhone` + `agentId` (nunca por `contextId`, nunca por heurística de "mais recente"), busca o histórico, gera um resumo estruturado via Groq (com regra anti-alucinação e priorização temporal) e envia ao Telegram — caindo no alerta simples já existente (`⚠️ RAFAEL, CLIENTE AGUARDANDO SEM RESPOSTA!`) sempre que qualquer etapa do resumo falhar. Dedup só é registrado depois que o Telegram confirma sucesso.

**Zero Serverless Functions novas** — mesmo padrão de consolidação de `qwen-health`/`nex-sync-clientes`/`stuck-check` em `api/system-tools.js`, pelo mesmo motivo (teto de 12 funções do Hobby); a primeira versão (`api/alerta-inteligente.js`, arquivo-rota próprio) chegou a estourar esse limite e teve o deploy rejeitado antes da migração.

**Estado real por agente (2026-08-13, confirmado por leitura direta da API do GPT Maker):** conectado só na **GABY LAB** (`agentId 3F78AF104664B0D1CB84D23672FCADC5`) — teste real final homologado. **Gabriela produção** (`agentId 3F4713FF6FA970BC0ED406900922C6C1`) continua com a intention apontando direto pra `api.telegram.org`, não conectada.

**Testado (2026-08-13):** 75/75 testes específicos; suíte completa 585 passando / 1 falha pré-existente e não relacionada.

Detalhe técnico completo (arquitetura, algoritmo de desambiguação, contrato do resumo, fallback, dedup, segurança, troubleshooting, checklist de reuso) em `docs/integrations/ALERTA-INTELIGENTE.md`. Decisões arquiteturais em `docs/decisions/0002-identificacao-chat-whatsappphone-agentid.md` e `docs/decisions/0003-alerta-inteligente-system-tools-telegram.md`.

---

## 7. Agrupamento funcional dos serviços (50 arquivos) — estrutura física real, 100% organizada

| Domínio (pasta) | Services | Arquivos |
|---|---|---|
| **`chat/`** | messageHistoryService, interactionsService, photoHistory, gptmaker | 4 |
| **`catalogo/`** | catalogSyncService, googleDriveCatalog, scraperService, scrapingService, catalog, catalogPublicConfig, normalizeProductName | 7 |
...
| **`ia/`** | deepseek, deepseekBalanceService, groq, qwenHealthService | 4 (nota: `openrouterBalanceService` também já existe em `ia/` e não estava listado aqui antes desta edição — gap pré-existente, não corrigido nesta tarefa) |
...
>
> **Atualizado 2026-07-25 (Operations Center — Fase 2A, QwenCloud health check):** adicionado `qwenHealthService.js` em `ia/`. **Limitação confirmada na auditoria original:** não existe API pública de saldo/quota do QwenCloud — o card mostra só disponibilidade, modelo configurado, latência da chamada de teste, última verificação e tokens gastos nesse próprio health check, nunca saldo/cota/limite de conta.
>
> **Atualizado 2026-07-25 (Operations Center — Fase 2A.1, persistência + trava atômica):** a Fase 2A original usava cache/rate-limit em memória do processo — auditoria de segurança apontou que isso não é confiável nem seguro em serverless (memória reseta a cada cold start, não é compartilhada entre instâncias/regiões, rota pública sem autenticação). Substituído por persistência real: migration `015_qwen_health_state.sql` (tabela `qwen_health_state` + função `claim_qwen_health_check`, ver `docs/SUPABASE.md §3.6` para a explicação completa da atomicidade). Fluxo:
> - **`GET`** → só lê o último estado persistido no Supabase. Nunca chama o QwenCloud. Custo externo zero, chamado ao montar `OperationsCenterPage.jsx` (via `qwenHealthService.getQwenHealthState()`), seguro em F5/navegação da SPA.
> - **`POST`** → chamado só pelo botão "Atualizar agora" (`qwenHealthService.requestQwenHealthRefresh()`). O endpoint tenta reivindicar o direito de chamar o QwenCloud via RPC `claim_qwen_health_check` — só uma instância consegue, mesmo sob concorrência real entre múltiplas instâncias/regiões da Vercel (atomicidade garantida por um único `UPDATE ... WHERE ... RETURNING`, não por lógica em JavaScript). Se a trava não liberar, devolve o último resultado persistido (`throttled: true`), sem gastar nenhuma chamada real.
> - **Intervalo mínimo configurável:** `QWEN_HEALTH_MIN_INTERVAL_SECONDS` (env var opcional, fallback 1800s/30min) — lido só dentro do endpoint, nunca fixado em múltiplos arquivos, nunca controlado pelo frontend.
> - **`enable_thinking: false`** adicionado na chamada ao QwenCloud — o modelo configurado é um modelo híbrido com raciocínio ligado por padrão, que ignorava `max_tokens` na fase de "thinking"; sem a flag, uma chamada mínima gastava 394 tokens, com a flag caiu pra ~14 (medido em teste real).
> - **Risco residual aceito:** o POST continua público, sem autenticação de usuário (o projeto não tem login hoje) — mitigado só pela trava atômica persistida. Documentado como pendência pra quando o projeto ganhar autenticação real.
> - **Credencial nova usada:** `SUPABASE_SECRET_KEY` (mesma já em uso por `api/_profileLearning.js` desde a Fase 2C — nenhuma variável nova precisou ser criada para o acesso ao Supabase).
>
> **Atualizado 2026-07-27 (consolidação — limite de 12 Functions do Hobby):** `api/qwen-health.js` foi **removido** e sua lógica migrada integralmente para dentro de `api/system-tools.js` como `?tool=qwen-health` (mesmo padrão já usado por `vercel-status`/`sync-lyra`/`stuck-check`/`lyra-webhook`/`gerar-cobranca-lyra` — um arquivo só, dividido por query param, pra caber no teto de 12 Serverless Functions do plano Hobby da Vercel). Comportamento idêntico ao da Fase 2A.1, só a URL mudou: `GET/POST /api/qwen-health` → `GET/POST /api/system-tools?tool=qwen-health`. `qwenHealthService.js` atualizado de acordo. Nenhuma outra tool de `system-tools.js` foi alterada. Contagem de arquivos roteáveis em `api/` voltou de 13 para 12.
>
> **Atualizado 2026-07-27 (OpenRouter — Pacote 1, saldo migrado pro server-side):** auditoria de segurança encontrou `VITE_OPENROUTER_API_KEY` exposta no bundle do frontend (confirmado: prefixo de chave `sk-or-` aparecia 3x no JS servido em produção), usada por `openrouterBalanceService.js` pra chamar `GET https://openrouter.ai/api/v1/credits` **direto do browser**. Corrigido: novo `?tool=openrouter-usage` em `api/system-tools.js` (GET só, sem auth de usuário — mesmo desenho do `vercel-status`, sem custo real por chamada e por isso **sem** Supabase/trava atômica, só cache em memória best-effort de 5min). `openrouterBalanceService.js` reescrito pra nunca mais ler variável de ambiente nem chamar `openrouter.ai` direto — só consulta o novo endpoint; interface legada (`getOpenRouterBalance()`) preservada intacta pro `DashboardNewPage.jsx`, que não teve nenhuma linha alterada. Nova função `getOpenRouterUsage(force)` alimenta o card real do Operations Center (`buildOpenRouterCard`, mesmo padrão do `buildQwenCard`) — mostra só saldo restante, gasto acumulado, créditos totais e última verificação; **não mostra requests/tokens/modelos usados**, por não haver fonte confirmada pra esses campos na API de créditos. Variável nova: `OPENROUTER_API_KEY` (server-side, sem prefixo `VITE_`) — ainda **não cadastrada em nenhum ambiente**, pendente de você adicionar em Preview antes do próximo deploy.
>
> **Atualizado 2026-07-27 (OpenRouter — Pacote 2, CODEX + OCR migrados):** `src/services/ia/groq.js` (fallback de modelo do CODEX, dentro de `askCODEX()`) e `src/services/foto/ocrService.js` (fallback de visão) paravam de ler `VITE_OPENROUTER_KEY` do bundle — migrados pra dois novos proxies fiéis em `api/system-tools.js`: `?tool=codex-openrouter` e `?tool=ocr-openrouter`. Proxy fiel: o servidor não decide prompt/modelo/regra de negócio, só repassa exatamente o corpo que o frontend já montava antes (mesmo `system prompt`, mesmo `temperature`, mesmo `max_tokens`), trocando só quem segura a chave — comportamento do CODEX e formato de resposta do OCR permanecem idênticos. Única diferença de segurança (não é funcionalidade nova): cada proxy valida `model` contra uma allowlist fixa espelhando exatamente os modelos já oferecidos na UI (7 modelos de chat em `DealOncaPage.jsx`, 2 modelos de visão no OCR) — sem isso, o endpoint viraria um relay aberto pra qualquer chamada cobrada na conta, sem nem precisar da chave (pior que a exposição anterior). **Confirmado por bundle: `sk-or-` caiu de 2 ocorrências pra 0** — zero chave OpenRouter embutida no JS de produção. Zero referência a `VITE_OPENROUTER_KEY`/`VITE_OPENROUTER_API_KEY` restante em `src/` (só um comentário histórico mencionando o nome antigo, não código). **Migração completa — projeto pronto para a rotação da chave** (`OPENROUTER_API_KEY` já é a única credencial em uso, 100% server-side); a rotação em si não foi feita nesta tarefa, por instrução explícita.
>
> **Atualizado 2026-07-29 (correção de duplicidade no sync de catálogo — normalização única):** novo arquivo `normalizeProductName.js` em `catalogo/` (7º arquivo do domínio) — fonte única de normalização de nome de produto, substituindo duas funções antes incompatíveis (`normalizarNomeProduto` em `catalogSyncService.js` corrigia grafia; `normalizarNome` em `conhecimento/knowledgeGenerator.js` só removia acento/caixa), que causavam duplicatas silenciosas em toda sincronização sempre que o nome de origem divergia da grafia já corrigida no banco. `catalogSyncService.js` e `knowledgeGenerator.js` passam a importar da mesma fonte. Ver `docs/WhatsApp-Premium.md` para o contexto completo do incidente que motivou a correção (não relacionado ao WhatsApp em si — foi corrigido na mesma sessão).
>
> **Atualizado 2026-07-27 (allowlist dinâmica — modelos gratuitos deixam de ser hardcoded):** a validação em Preview do Pacote 2 confirmou que **7 dos 9 IDs de modelo gratuito hardcoded já não existiam mais** no catálogo do OpenRouter (confirmado com chamadas reais retornando 404/400 do próprio OpenRouter). Causa: o OpenRouter renomeia/descontinua variantes `:free` com frequência. Corrigido substituindo as allowlists fixas por uma allowlist dinâmica em `api/system-tools.js`: `getOpenRouterAllowlist()` consulta `GET https://openrouter.ai/api/v1/models` (**endpoint público, não usa `OPENROUTER_API_KEY`**), filtra por `pricing.prompt/completion === 0` + modalidade certa (texto pro CODEX, texto+imagem pro OCR), com **cache de 12h** e **último snapshot válido preservado além do TTL** (`openrouterModelsLastGood`) — se o catálogo ficar inacessível, usa o último bom conhecido; se nunca houve nenhum fetch bem-sucedido (cold start + catálogo fora do ar), usa uma pequena lista de emergência hardcoded (2 modelos confirmados manualmente nesta auditoria). `curateCodexModels()`/`curateOcrModels()` limitam a exibição/permissão a no máximo 10 modelos de texto (+ `openrouter/free`, o router automático oficial do OpenRouter, sempre por último e nunca padrão) e 3 modelos de visão — a allowlist server-side continua sendo a autoridade final (o POST revalida independente do que o GET listou). Os dois proxies (`codex-openrouter`/`ocr-openrouter`) ganharam um branch **GET** (mesmo tool, mesmo padrão de `qwen-health`) que devolve a allowlist atual — consumido por `DealOncaPage.jsx` (dropdown do CODEX, antes 7 entradas fixas) e `ocrService.js` (loop de fallback do OCR, antes array fixo de 2). `groq.js` não precisou de nenhuma mudança (já só repassa o `modelId` escolhido, nunca conheceu a lista). Nenhuma migration, nenhuma Serverless Function nova — contagem de `api/` continua 12.
...
10. **⚠️ Dívida técnica de segurança — chave Base44 exposta no bundle do frontend** (`src/services/crm/cobrancasService.js`): `VITE_BASE44_API_KEY`/`VITE_BASE44_APP_ID` são injetadas no bundle público pelo Vite e dão acesso de leitura/escrita irrestrito a `Cliente`/`Parcela`/`HistoricoAtividade` do app PRIME Cobranças (auditoria completa em 2026-07-28: risco classificado como **Alto**, arquitetura de migração para server-side já desenhada e aprovada, mas **não implementada por decisão de projeto** — app de uso interno, prioridade em outras funcionalidades no momento). **Migrar o acesso Base44 do frontend para server-side antes da finalização definitiva do projeto e antes da rotação da chave.** Não remover `VITE_BASE44_*` nem rotacionar a chave antes dessa migração (rotacionar antes não resolve nada e arrisca quebrar `api/system-tools.js`, que usa a mesma chave sem prefixo `VITE_`).
11. **✅ Resolvido (2026-07-28) — `base44/.app.jsonc` estava vinculado a um appId órfão**: corrigido. `base44/.app.jsonc` agora aponta oficialmente para **PRIME Cobranças Starter** (`6a50402b2eeb1d1114312861`) — o appId órfão anterior (`6a2e32484d9cd88185e81e18`) não corresponde a nenhum app conhecido e foi descartado (backup preservado durante a migração, depois removido após confirmação). `base44/functions/` foi comparado byte a byte contra as 10 Functions reais publicadas no PRIME Starter (`functions pull` pra um snapshot isolado, nunca sobrescrevendo o repositório diretamente) e agora contém exatamente essas 10, todas confirmadas idênticas ao remoto: `whatsappProvider`, `lembreteCobrancas`, `enviarConfirmacaoPagamentoWhatsapp`, `enviarMensagemManualWhatsapp` (já existiam localmente, confirmadas sem divergência) + `notificacaoCore`, `syncParcelasCalendar`, `resumoCobrancasAtrasadas`, `resumoVencimentosDia`, `portalCliente`, `notificacaoScheduler` (trazidas do remoto nesta organização). **`consultarStatusCobranca` não pertence ao PRIME** — confirmado (via `functions list` contra o app Lyra) que é uma Function do app **Lyra** (`6a518d72335f3c31663dc63d`), hoje isolada em `base44/lyra-functions/consultarStatusCobranca/` (sem imports relativos quebrados — só depende do pacote `npm:@base44/sdk`). `base44/functions/` representa exclusivamente o PRIME Starter a partir de agora.
12. **Rate limit do Fluxo I (NEX) é best-effort, não é garantia de segurança** — `Map` em memória por IP, resetado em cold start, não compartilhado entre instâncias da Vercel (mesma ressalva já registrada para os Fluxos F.1/H). A proteção real do endpoint `?tool=nex-sync-clientes` é a autenticação por `NEX_SYNC_SECRET`, não o rate limit.
13. **PRIME Bridge (`poc/zap-gptmaker-bridge/`) é um processo standalone fora do diagrama da seção 1** — não faz parte do fluxo de requisição do app principal (não é `api/` nem `src/`), mas passou a compartilhar o mesmo projeto Supabase como consumidor server-side desde a Fase 2B.2 (`bridge_message_processing`, `bridge_operation_logs`, migrations 017/018 — ver `docs/SUPABASE.md §3.8`). Roda localmente atrás de um túnel Cloudflare, não hospedado na Vercel; ainda em fase de validação (POC), não substituiu o canal nativo do GPTMaker em produção. Detalhe completo em `docs/integrations/PRIME-BRIDGE-FASE2.md`.
14. **Alerta Inteligente (Fluxo J, seção 6) homologado só na GABY LAB, não em produção** — a intention "Alerta rafael" da Gabriela de produção continua chamando `api.telegram.org` direto (não migrada); conectar é uma decisão separada, ainda não tomada. Não presumir que a migração vale para todos os agentes sem confirmar por leitura direta da API do GPT Maker. Detalhe completo em `docs/integrations/ALERTA-INTELIGENTE.md §4`.
15. **Consulta_Frete_API homologada só na GABY LAB, via ação manual do GPT Maker (não a conexão MCP nativa)** — ferramenta `consultar_frete` (`api/_consultarFrete.js`, registrada em `api/system-tools.js` junto de `consultar_cep`/`consultar_cobrancas`) validada ponta a ponta contra a Frenet real e em conversa real via WhatsApp. Achado reutilizável: nomes de header sem `:` no final na tela de ação manual do GPT Maker (`Content-Type:`/`Authorization:` quebravam o executor `okhttp` do GPT Maker com `stream was reset: CANCEL`, antes de qualquer coisa chegar à Vercel). Padrão (`GPT Maker ação manual → JSON-RPC tools/call → /api/system-tools?tool=mcp`) documentado como reaproveitável para outras ferramentas MCP existentes, nenhuma ativada além desta. Detalhe completo em `docs/integrations/CONSULTA-FRETE-API.md`.
...
**Atualizado em:** 2026-07-14 · Fase 3.2.0 — CORS seguro em `?tool=gerar-cobranca-lyra` (Fluxo F.1): preflight `OPTIONS` suportado, allowlist por igualdade exata contra `GERAR_COBRANCA_ALLOWED_ORIGINS` (nunca `startsWith`), `Authorization`/`Content-Type` liberados via `Access-Control-Allow-Headers`, wildcard `*` global removido só dentro deste `case` (as outras 4 tools preservam o `*` de sempre, sem mudança de comportamento). Validado em produção: preflight de origem permitida e de 3 variações maliciosas (subdomínio forjado, path embutido, protocolo `http://`) — todas bloqueadas corretamente. Prepara o terreno pra Fase 3.2 (service no Base44 chamando este endpoint direto do navegador).
**Atualizado em:** 2026-07-14 · Fase 3.3.1/3.3.2 — token temporário do frontend (`COBRANCA_FRONTEND_TOKEN`, Fluxo F.1): o Base44 Starter não expõe Secrets do backend pro frontend, então foi criado um segundo segredo, público por design, isolado do `GERAR_COBRANCA_SECRET` (nunca autentica nenhuma outra tool), com restrições exclusivas — body estritamente `{parcela_id, dryRun:false}`, Origin exato obrigatório, rate limit próprio mais restritivo (5/min por IP, 2/5min por parcela). Lógica financeira 100% compartilhada com o modo admin, sem alteração. Validado em produção (auth dupla, rejeição em `sync-lyra`/`lyra-webhook`, rate limit, CORS, zero escrita) — auditoria prévia (Fase 3.3.1A) não encontrou risco bloqueante, 2 achados médios corrigidos antes da publicação (Fase 3.3.1B). Uso real no frontend fica condicionado ao PRIME STORE - COBRANÇAS estar restrito a usuários convidados no Base44.
**Atualizado em:** 2026-07-29 · WhatsApp Premium (Fluxo H, seção 6) — novo fluxo de envio manual de WhatsApp dentro de Cobranças (`WhatsAppSendModal.jsx`, mensagem pronta com template ou personalizada via `EnviarMensagemManualModal.jsx` já existente), reaproveitando o proxy `?tool=mensagem-manual`/`api/_mensagemManualProxy.js` já validado, com dois novos `case` de leitura (`listar_templates`/`previsualizar`) intercalados antes do envio. Validado em produção com cliente de teste identificado, nas duas variantes, sem erro de console/rede. Detalhes completos em `docs/WhatsApp-Premium.md`. Na mesma sessão, corrigida a duplicidade silenciosa no sync de catálogo (novo `catalogo/normalizeProductName.js`, seção 7) — não relacionado ao WhatsApp, mas implantado nos mesmos dois commits (`5493a91`, `2fead88`).
**Atualizado em:** 2026-07-31 · Fase 6C — Integração NEX (Fluxo I, seção 6; item 12 da seção 8): novas tabelas `nex_clientes`/`nex_sync_eventos` (migration `016_nex_clientes.sql`, versionada e **aplicada manualmente no Supabase Production em 2026-07-31**, tabelas criadas vazias com RLS habilitada), staging isolado de clientes importados do sistema NEX externo, com chave natural `(origem_loja, nex_codigo)`, idempotência por `content_hash` e soft-delete via `ausente_desde`. Três novos `case` em `api/system-tools.js` (`nex-sync-clientes`, `nex-cliente`, `nex-health`) delegando 100% da lógica a `api/_nexClientes.js` (helper privado novo) — zero Serverless Functions novas, contagem de `api/` inalterada; **código desses handlers ainda não foi deployado na Vercel**. RLS zero-policy em `nex_clientes`/`nex_sync_eventos` — mesmo padrão de `qwen_health_state` (015); `profile_learning_audit` (013/014) é parecida mas não idêntica (tem uma policy de SELECT liberada, não é zero-policy) — correção da nota na `docs/SUPABASE.md §4` que dizia "as únicas duas". `NEX_SYNC_SECRET` isolado, só em Vercel Production, com cópia em cofre local protegido do macOS. 55/55 testes locais passando (mock de Supabase). Isolamento total do Base44 confirmado por auditoria estrutural. **Deploy do código, POC controlada em Production, validação real de RLS via anon key, cleanup e rollback ainda não executados** — ver `docs/integrations/NEX-INTEGRATION.md`.
**Atualizado em:** 2026-07-31 · PRIME Bridge — Fase 2A a 2B.5 (item 13 da seção 8): `poc/zap-gptmaker-bridge/server.mjs` (processo standalone, fora do diagrama da seção 1) ganhou `LIVE_MODE` seguro por padrão, timeout/normalização de telefone/mascaramento/taxonomia de erro (Fase 2A), proteção do webhook por path secreto (Fase 2B.1), dedupe persistente via `bridge_message_processing`/RPC `process_bridge_message` (Fase 2B.2-2B.3) e logging persistente fire-and-forget via `bridge_operation_logs` (Fase 2B.4) — migrations `017`/`018`, ver `docs/SUPABASE.md §3.8`. Arquitetura simplificada (sem posse por tentativa/lease) deliberadamente escolhida por a bridge rodar como processo único, sem concorrência entre instâncias. 13 blocos de teste integrado (Fase 2B.5) aprovados contra o `server.mjs` real com GPTMaker/ZAP-API/Supabase totalmente mockados localmente, zero chamada externa real. Ainda em fase de validação — não substituiu o canal nativo do GPTMaker em produção, nenhum teste real de WhatsApp realizado desde as mudanças da Fase 2. Detalhe completo em `docs/integrations/PRIME-BRIDGE-FASE2.md`.
**Atualizado em:** 2026-08-01 · Fase 6C encerrada (Fluxo I, seção 6) — supersede a entrada de 2026-07-31 acima (Fase 6C — Integração NEX), que ficou desatualizada. O deploy inicial (commit `faffb99`) chegou a Production com um bug: os 3 handlers NEX usavam `createClient(SUPABASE_URL, SUPABASE_SECRET_KEY)`, mas esse nome já estava importado do `@base44/sdk` no topo do arquivo — o código chamou por engano o client do Base44, gerando `500`/`"supabase.from is not a function"` em Production (não pego pelos 55 testes anteriores, que mockavam uma API chainable do Supabase-JS inexistente neste projeto). Corrigido no commit `a595ab5` (Fase 6C.6): migração completa para REST via `fetch` (mesmo padrão de `_profileLearning.js`/`qwenHealthSupabaseHeaders()`), lógica concentrada em `api/_nexClientes.js`, zero `createClient` no fluxo NEX. Cobertura de regressão adicionada: 70/70 testes, incluindo 4 testes comportamentais que exercitam o `default export` real de `system-tools.js` e confirmam ausência de `createClient` da Base44 nos 3 handlers. POC controlada (Fase 6C.7) executada em Production com dados fictícios (`origem_loja='primestore-udi-poc-teste'`) cobrindo criação/atualização/idempotência/erro/consulta/agregados, encerrada com cleanup validado (0 clientes/0 eventos). Pendências de baixa prioridade (não bloqueiam produção): `updated_at` não atualizado no upsert; formato exato da `SUPABASE_SECRET_KEY` não confirmado formalmente. Detalhe completo em `docs/integrations/NEX-INTEGRATION.md`.
**Atualizado em:** 2026-08-13 · Alerta Inteligente de Handoff Humano homologado (Fluxo J, seção 6; item 14 da seção 8) — resumo estruturado via Groq (regra anti-alucinação + priorização temporal) enviado ao Telegram quando a Gaby pede atendimento humano, identificação determinística do chat por `whatsappPhone`+`agentId` (nunca heurística de recência — ver ADR 0002), fallback simples sempre que qualquer etapa do resumo falhar, dedup só após confirmação do Telegram. Nasceu como arquivo-rota próprio (`api/alerta-inteligente.js`), estourou o limite de 12 Serverless Functions do Hobby e teve o deploy rejeitado; migrado para `?tool=alerta-inteligente` em `api/system-tools.js` + `api/_alertaInteligente.js` (ver ADR 0003) — contagem de `api/` voltou a 12. Homologado por 3 testes reais (ambiguidade de telefone → correção `agentId` → priorização temporal → passou), **só no agente GABY LAB** — Gabriela produção não conectada, confirmado por leitura direta da API do GPT Maker, não presumido. 75/75 testes específicos, suíte completa 585/586 (1 falha pré-existente não relacionada). Detalhe completo em `docs/integrations/ALERTA-INTELIGENTE.md`.
