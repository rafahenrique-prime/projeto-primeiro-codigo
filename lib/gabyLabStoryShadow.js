...
function selecionarProdutoUnico(produtos, contexto) {
  ...
  if (exato) {
    const restantes = candidatos.filter((produto) => produto !== exato)
    return {
      status: 'clear',
      produto: exato,
      candidatos,
      topScore: Number(exato?.relevancia ?? 0),
      secondScore: Number(restantes[0]?.relevancia ?? 0),
    }
  }
  ...
  if (!top) return { status: 'no_match', produto: null, candidatos, topScore, secondScore }
  if (!segundo) return { status: 'clear', produto: top, candidatos, topScore, secondScore }

  const vantagemForte = topScore >= 50 && (topScore - secondScore >= 30 || topScore >= secondScore * 1.5)
  return vantagemForte
    ? { status: 'clear', produto: top, candidatos, topScore, secondScore }
    : { status: 'ambiguous', produto: null, candidatos, topScore, secondScore }
}
...
function respostaClarificacao(pergunta, trace, { motivo, categoria = null, marca = null, cor = null } = {}) {
  ...
  const ehUnicoAmbiguo = motivo === 'single_product_ambiguous' || motivo === 'single_product_not_found'
...
export async function runGabyLabStoryShadow(req, res) {
  ...
  if (trace.search_context_used === 'story' || trace.search_context_used === 'story_clarified' || trace.search_context_used === 'story_single') {
    ...
    if (trace.search_context_used === 'story_single' && contextoUnico) {
      ...
      if (selecao.status === 'clear' && selecao.produto) {
        return res.status(200).json(respostaComPerguntaOriginal(shadow.body, pergunta, trace, [selecao.produto]))
      }

      trace.clarification_needed = true
      trace.clarification_reason = selecao.status === 'no_match' ? 'single_product_not_found' : 'single_product_ambiguous'
      trace.search_context_used = 'story_clarification'
      return res.status(200).json(respostaClarificacao(pergunta, trace, {
