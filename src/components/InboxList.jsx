    ...
    <div style={{ width: 300, background: t.bg, borderRadius: 12, display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ padding: showFilters ? '16px 14px 12px' : '14px 14px 10px' }}>
        ...
        <div style={{ position: 'relative', marginBottom: 10, display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          >
            {sleepLoading ? <LoadingIcon /> : <SparkIcon asleep={botSleep} />}
          </button>
        </div>
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          {humanCount > 0 && (
            <button onClick={() => setFilter(filter === 'human' ? 'all' : 'human')} style={{
              ...
              <AlertIcon color={filter === 'human' ? '#fff' : '#DC2626'} /> Humano
          ...
          {Object.entries(igColors).map(([channelId, color], i) => {
            ...
            return (
              <button key={key} onClick={() => setFilter(isActive ? 'all' : key)} style={{
                ...
                fontWeight: isActive ? 600 : 500, transition: 'all 0.12s',
                display: 'flex', alignItems: 'center', gap: 4,
              }}>
          ...
          <button onClick={() => setFilter(filter === 'ig' ? 'all' : 'ig')} style={{
            ...
            fontWeight: filter === 'ig' ? 600 : 500, transition: 'all 0.12s',
            display: 'flex', alignItems: 'center', gap: 4,
          }}>
...
function ConvItem({ conv, isActive, onClick, t, igColors = {}, buyScore }) {
  const hasScore = buyScore != null
  const progress = hasScore ? buyScore : (conv.objective_progress || 0)
  const isWa = conv.channel === 'whatsapp'
  const igBadgeColor = igColors[conv.igChannelId || conv.channelId] || '#E1306C'
  const ringColor = hasScore ? scoreColor(buyScore) : (isWa ? '#0EC331' : igBadgeColor)
    ...
    >
      ...
      <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {hasScore && (
          <span style={{
            ...
            borderRadius: 9999, padding: '1px 6px', transition: 'background 0.5s ease',
          }}>{buyScore}%</span>
        )}
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          {needsHuman
            ? <span style={{ fontSize: 10, fontWeight: 600, color: '#DC2626', background: '#FEE2E2', borderRadius: 6, padding: '1px 6px', display: 'flex', alignItems: 'center', gap: 3 }}><AlertIcon color="#DC2626" size={9} /> Aguardando</span>
            : <span style={{ fontSize: 10, fontWeight: 500, color: conv.mode === 'copilot' ? '#6366F1' : '#00A84F', background: conv.mode === 'copilot' ? '#EEF2FF' : '#F0FDF4', borderRadius: 6, padding: '1px 6px' }}>{conv.mode === 'copilot' ? 'Copilot' : 'AutoPilot'}</span>
          }
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {hasScore && (buyScore >= 70 || buyScore < 40) && (
              <span style={{ fontSize: 12 }}>{buyScore >= 70 ? '🔥' : '🧊'}</span>
            )}
            {waitTime && (
              <span style={{
                fontSize: 10, fontWeight: 600, borderRadius: 6, padding: '1px 5px',
                background: isLong ? '#FEE2E2' : '#FEF3C7',
                color: isLong ? '#DC2626' : '#D97706',
                display: 'flex', alignItems: 'center', gap: 2,
              }}><ClockIcon size={9} />{waitTime}</span>
            )}
            {conv.unread > 0 && (
              <span style={{ background: t.primary || '#E8192C', color: '#fff', fontSize: 10, fontWeight: 600, borderRadius: 9999, minWidth: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 5px' }}>{conv.unread}</span>
            )}
          </div>
        </div>
      </div>
...
function AlertIcon({ color = '#DC2626', size = 12 }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
}
...
function LoadingIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{ animation: 'spin 0.8s linear infinite' }}><path d="M21 12a9 9 0 1 1-9-9"/></svg>
}
function SparkIcon({ asleep }) {
  if (asleep) return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z"/></svg>
  return (
