...
const TYPE_META = {
  ...
  semelhante: { label: 'Semelhante', color: '#6366F1', icon: '🪞' },
  obsoleto: { label: 'Obsoleto', color: '#F59E0B', icon: '🗓️' },
  muito_curto: { label: 'Muito curto', color: '#9CA3AF', icon: '✂️' },
  contraditorio: { label: 'Contraditório', color: '#DB2777', icon: '⚡' },
}

export default function KnowledgeAuditTab({ t }) {
  ...
  // autoSelect só deve acontecer no carregamento inicial da aba — depois de rodar uma
  // auditoria nova, já mostramos o resultado local e só atualizamos a lista do histórico.
  ...
  async function handleRun() {
    ...
    setProgress(null)
    try {
      const out = await runKnowledgeAudit((done, total) => setProgress({ done, total }))
      // Mostra o resultado que acabou de calcular direto, sem depender do round-trip
      // de volta pro Supabase (evita tela "zerada" se o insert de histórico falhar).
  ...
  async function handleIgnore(item, ignored) {
    // Resultado local (rodada ainda não persistida) — só atualiza a tela.
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: t.text, margin: 0 }}>📚 Auditoria da Base de Conhecimento</h2>
        <button onClick={handleRun} disabled={running} style={{
          ...
          {running
            ? progress?.total ? `Julgando contradições... ${progress.done}/${progress.total}` : 'Comparando embeddings...'
            : 'Rodar auditoria agora'}
        </button>
      </div>
      <p style={{ fontSize: 12, color: t.textMuted, marginTop: 0, marginBottom: 20 }}>
        Duplicados, semelhantes e conteúdo curto/obsoleto são achados por regra + embeddings já salvos na base (sem custo de IA).
        Contradição usa IA (DeepSeek R1) só nos pares mais parecidos, com citação literal exigida como evidência.
      </p>
    ...
    <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: 10, padding: '12px 14px', opacity: r.ignored ? 0.6 : 1 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {r.score != null && <span style={{ fontSize: 11, color: t.textMuted }}>similaridade {(r.score * 100).toFixed(1)}%</span>}
        </div>
      ...
      <div style={{ fontSize: 13, color: t.text, fontWeight: 600 }}>{r.title_a}{r.title_b ? ` ⟷ ${r.title_b}` : ''}</div>
