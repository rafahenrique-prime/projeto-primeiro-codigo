...
export default function InstagramAuditTab({ t }) {
  ...
  async function handleRun() {
    ...
    setError(null)
    try {
      const out = await runInstagramAudit()
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: t.text, margin: 0 }}>📸 Auditoria de Instagram</h2>
      ...
      <p style={{ fontSize: 12, color: t.textMuted, marginTop: 0, marginBottom: 20 }}>
        100% regra (sem custo de IA): DMs sem resposta, contatos duplicados, sem nome, abandonadas e sem interação recente.
        Comentários sem resposta e perguntas recorrentes ainda não têm fonte de dado — o GPT Maker só expõe DMs.
      </p>
...
function FindingCard({ r, t, onIgnore }) {
  ...
  return (
    <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: 10, padding: '12px 14px', opacity: r.ignored ? 0.6 : 1 }}>
      ...
      <div style={{ fontSize: 13, fontWeight: 600, color: t.text }}>{r.contact_name || '—'} {r.username && <span style={{ fontWeight: 400, color: t.textMuted }}>· @{r.username}</span>}</div>
