...
const TYPE_META = {
  ...
  funcao_sem_uso: { label: 'Função sem uso', color: '#F59E0B', icon: '🧩' },
  componente_duplicado: { label: 'Componente duplicado', color: '#7C3AED', icon: '🧬' },
  rota_morta: { label: 'Rota morta', color: '#DB2777', icon: '🚪' },
  tabela_sem_uso: { label: 'Tabela sem uso', color: '#9CA3AF', icon: '🗄️' },
}

export default function CodexAuditTab({ t }) {
  ...
  async function loadRuns() {
    ...
    setRuns(r)
    if (r.length > 0) await selectRun(r[0].run_id)
    setLoading(false)
  }
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: t.text, margin: 0 }}>🧩 Auditoria de Código (CODEX)</h2>
      </div>
      <p style={{ fontSize: 12, color: t.textMuted, marginTop: 0, marginBottom: 20 }}>
        Esta auditoria não roda dentro do navegador — o browser não tem acesso ao filesystem do repositório.
        Os achados são gerados pelo Claude Code analisando o código-fonte e gravados aqui; peça uma nova rodada quando quiser atualizar.
      </p>
        ...
        <>
          {activeRun && (
            <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
              <div style={{ fontSize: 11, color: t.textMuted, marginBottom: 8 }}>
                Última análise: {activeRunMeta ? new Date(activeRunMeta.created_at).toLocaleString('pt-BR') : '—'}
              </div>
          ...
          {runs.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, color: t.textMuted, fontWeight: 600, textTransform: 'uppercase', marginBottom: 6 }}>Histórico de análises</div>
...
function FindingCard({ r, t, onIgnore }) {
  ...
  return (
    <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: 10, padding: '12px 14px', opacity: r.ignored ? 0.6 : 1 }}>
      ...
      {r.path && <div style={{ fontSize: 12.5, fontFamily: 'monospace', color: t.text, marginBottom: 4 }}>{r.path}</div>}
      {r.detail && <div style={{ fontSize: 12, color: t.textMid }}>{r.detail}</div>}
    </div>
  )
}
