...
const TYPE_META = {
  ...
  conflitante: { label: 'Conflitante', color: '#DB2777', icon: '⚡' },
  muito_curta: { label: 'Muito curta', color: '#9CA3AF', icon: '✂️' },
}

export default function LearningsAuditTab({ t }) {
  ...
  async function handleRun() {
    ...
    setProgress(null)
    try {
      const out = await runLearningsAudit((done, total) => setProgress({ done, total }))
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: t.text, margin: 0 }}>🧠 Auditoria de Aprendizagem</h2>
        <button onClick={handleRun} disabled={running} style={{
          ...
          {running
            ? progress?.total ? `Julgando conflitos... ${progress.done}/${progress.total}` : 'Comparando aprendizados...'
            : 'Rodar auditoria agora'}
        </button>
      </div>
      <p style={{ fontSize: 12, color: t.textMuted, marginTop: 0, marginBottom: 20 }}>
        Duplicadas e muito curtas são achadas por regra (similaridade de palavras, sem custo de IA).
        Conflito usa IA (DeepSeek R1) só nos pares mais parecidos, com citação literal exigida como evidência.
      </p>
...
function FindingCard({ r, t, onIgnore }) {
  ...
  return (
    <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: 10, padding: '12px 14px', opacity: r.ignored ? 0.6 : 1 }}>
      ...
      <div style={{ fontSize: 13, color: t.text }}>{r.content_a}</div>
      {r.content_b && <div style={{ fontSize: 13, color: t.textMid, marginTop: 4 }}>⟷ {r.content_b}</div>}
      {r.detail && <div style={{ fontSize: 12, color: t.textMid, marginTop: 4, fontStyle: 'italic' }}>{r.detail}</div>}
    </div>
  )
}
