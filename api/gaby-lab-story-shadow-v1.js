...
// LAB only. Best-effort cache local à instância da Function: evita reprocessar
// o mesmo Story em continuidades curtas quando a instância permanece quente.
// Não é usado pela GABY OFICIAL e não substitui um cache persistente futuro.
...
function getCachedQuery(storyId, now = Date.now()) {
  ...
  const cached = storyQueryCache.get(String(storyId))
  if (!cached) return null
  if (cached.expiresAt <= now) {
    storyQueryCache.delete(String(storyId))
    return null
  }
  return cached.query
}
...
async function buscarShadow(pergunta) {
  ...
  try {
    ...
    clearTimeout(timeout)
    return { ok: false, status: 502, error: err?.name === 'AbortError' ? 'SHADOW_TIMEOUT' : 'SHADOW_UNAVAILABLE' }
  }
}
...
export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ sucesso: false, error: 'METHOD_NOT_ALLOWED' })
  if (req.headers?.['x-prime-lab'] !== LAB_HEADER) return res.status(403).json({ sucesso: false, error: 'FORBIDDEN_LAB_ONLY' })

  const pergunta = String(req.body?.pergunta ?? '').trim().slice(0, 300)
  const chatId = typeof req.body?.chat_id === 'string' ? req.body.chat_id.trim() : ''
  if (pergunta.length < 2) return res.status(400).json({ sucesso: false, error: 'PERGUNTA_REQUIRED' })
  ...
  if (!shadow.ok) return res.status(shadow.status).json({ sucesso: false, error: shadow.error })
  ...
  if (trace.search_context_used === 'story') {
    ...
    if (confiaveis.length === 0) {
      ...
      // Só cacheia uma Vision NOVA depois que o Shadow V2 confirmou pelo menos
      // um candidato forte. Evita amplificar uma identificação visual errada.
