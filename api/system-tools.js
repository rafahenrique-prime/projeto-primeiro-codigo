...
// TEMP POC (remover após Passo 0 de suporte a Story em vídeo) — ver tool=debug-ffmpeg-frame
...
// TEMP POC — Passo 0 isolado: comprova que ffmpeg-static extrai 1 frame de um
// Story real que chegou como video/mp4 (foto+música do Instagram) e que esse
// frame é identificável pela mesma visão já homologada. URL do Story é fixa
// no código (nunca vem de req.body) — sem risco de injeção. Argumentos do
// ffmpeg são um array fixo, execFile (nunca shell). Nunca loga a URL nem
// base64. Apaga os arquivos temporários ao final. NÃO integrado ao fluxo real.
const DEBUG_STORY_VIDEO_URL = 'https://gpt-files.com/file/3F306ABF4F19303E3DC66E0E9A5EC274/3F872365DAE220238BB626617B53CA57/AQNsI5l5Tv2FPjfvkcepgHBrWzCYczpLffnxgJx4dqzph1Nc1d3t32ZjuEXw5s_y4xk6LwCd6jlwJ8iNMvpql56y7_mfyj0IyBtlY68.mp4'
const DEBUG_ALLOWED_HOST = 'gpt-files.com'
const DEBUG_MAX_VIDEO_BYTES = 20 * 1024 * 1024

const DEBUG_PROMPT_IDENTIFICACAO = `Você é um especialista em identificação de produtos para lojas.
...
async function debugFfmpegFrame(req, res) {
  const tempFiles = []
  const t0 = Date.now()
  const timings = {}

  try {
    const u = new URL(DEBUG_STORY_VIDEO_URL)
    if (u.protocol !== 'https:' || u.hostname !== DEBUG_ALLOWED_HOST) {
      return res.status(400).json({ ok: false, etapa: 'validacao_url', erro: 'host nao permitido' })
    }

    const tDownloadStart = Date.now()
    const videoRes = await fetch(DEBUG_STORY_VIDEO_URL, { redirect: 'manual' })
    ...
    if (videoBuffer.byteLength > DEBUG_MAX_VIDEO_BYTES) {
      ...
      const visionRes = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: [
            { type: 'text', text: DEBUG_PROMPT_IDENTIFICACAO },
...
export default async function handler(req, res) {
  ...
  switch (req.query.tool) {
    ...
    case 'debug-ffmpeg-frame':
      // TEMP POC — Passo 0 de suporte a Story em vídeo (foto+música do Instagram
      // chega como video/mp4). Não integrado a webhook.js/_storyContext.js/Action
      // real. Remover após validação.
      return debugFfmpegFrame(req, res)
